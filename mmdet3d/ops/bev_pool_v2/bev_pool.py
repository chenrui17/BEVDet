...
class QuickCumsumCuda(torch.autograd.Function):
    ...
    def forward(ctx, depth, feat, ranks_depth, ranks_feat, ranks_bev,
        ...
        with nvtx.annotate("type_to_int", color="orange"):
    ...
    #     with nvtx.annotate("back_ward_contiguous", color="gray"):
    ...
    def backward(ctx, out_grad):
        ...
        with nvtx.annotate("back_ward_argsort", color="yellow"):
            order = ranks_feat.argsort()
            # values, order = torch.topk(ranks_feat, k=ranks_feat.shape[0], largest=False)
        # print("order:",order)
        with nvtx.annotate("rank_process", color="orange"):
            ranks_feat, ranks_depth, ranks_bev = \
                ranks_feat[order], ranks_depth[order], ranks_bev[order]
        ...
        feat_grad = feat.new_zeros(feat.shape)
        out_grad = out_grad.contiguous()
        # print("interval_lengths_bp shape:",interval_lengths_bp.shape)
        # print("interval_lengths_bp max:", max(interval_lengths_bp))
        # print("interval_lengths_bp min:",min(interval_lengths_bp))
        # print("interval_starts_bp shape:",interval_starts_bp.shape)
        with nvtx.annotate("backward", color="red"):
...
def bev_pool_v2(depth, feat, ranks_depth, ranks_feat, ranks_bev,
                bev_feat_shape, interval_starts, interval_lengths):
    with nvtx.annotate("apply", color="yellow"):
...
def test_bev_pool_v2():
    ...
    with nvtx.annotate("init data and H2D", color="green"):
    ...
    with nvtx.annotate("bev_pool_v2", color="gray"):
        bev_feat = bev_pool_v2(depth, feat, ranks_depth, ranks_feat, ranks_bev, (1, 1, 2, 2, 2), interval_starts, interval_lengths)
    loss = torch.sum(bev_feat)
    with nvtx.annotate("bwd", color="blue"):
        loss.backward()
    print("loss:",loss)
    assert loss == 4.4
...
def test_bev_pool_v2_real_data():
    depth = torch.load('/home/auto/charlie/bev_pool_inputs/depth.pt').cuda().requires_grad_(True)
    feat = torch.load('/home/auto/charlie/bev_pool_inputs/feat.pt').cuda().requires_grad_(True)
    ranks_depth = torch.load('/home/auto/charlie/bev_pool_inputs/ranks_depth.pt').cuda().requires_grad_(False)

    ranks_feat = torch.load('/home/auto/charlie/bev_pool_inputs/ranks_feat.pt').cuda().requires_grad_(False)

    ranks_bev = torch.load('/home/auto/charlie/bev_pool_inputs/ranks_bev.pt').cuda().requires_grad_(False)

    bev_feat_shape = (12, 1, 192, 256, 128)
    interval_starts = torch.load('/home/auto/charlie/bev_pool_inputs/interval_starts.pt').cuda().requires_grad_(False)
    interval_lengths = torch.load('/home/auto/charlie/bev_pool_inputs/interval_lengths.pt').cuda().requires_grad_(False)
    # print("interval_lengths")
    grad_y = torch.ones(12, 128, 1, 192, 256, device='cuda')
    for iter in range(5):
        bev_feat = bev_pool_v2(depth, feat, ranks_depth, ranks_feat, ranks_bev, bev_feat_shape, interval_starts, interval_lengths)
        # bev_feat.backward(grad_y)
        loss = torch.sum(bev_feat)
        loss.backward()
        print("loss:",loss)
